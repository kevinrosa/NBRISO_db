...
nc_filename = '/Users/kevinrosa/GSO/BayCirculation/NBFSMN_through2014_hrmn_rosa_v02.nc';
...
for i = 1:length(mat_file)
    clearvars locn hrmn hrmn  % make sure previous 
    load(fullfile(data_dir, mat_file(i).name))
    
    t_start = min(hrmn.dnest(1), t_start);
    t_end   = max(hrmn.dnest(end), t_end);
end
%dt  = hrmn.dnest(2) - hrmn.dnest(1);
dt = datenum(0000,00,00,1,0,0);
...
for i = 1:length(mat_file)
    clearvars locn hrmn hrmn  % make sure previous load is overwritten
    load(fullfile(data_dir, mat_file(i).name))
   ...
   % site_abrv   = locn.abrv;
    ...
    temp(i, index, 1)   = hrmn.sh.temp;
    temp(i, index, 2)   = hrmn.dp.temp;
    sal(i, index, 1)    = hrmn.sh.sal;
    sal(i, index, 2)    = hrmn.dp.sal;
    domgl(i, index, 1)  = hrmn.sh.domgl;
    domgl(i, index, 2)  = hrmn.dp.domgl;
    chl(i,index)        = hrmn.sh.chl;
end
